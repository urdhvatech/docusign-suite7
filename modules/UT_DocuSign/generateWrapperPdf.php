...
function UTGetWrapperPDFContent($templateID, $task='pdf', $module, $uid)
{
    ...
    $text = populate_group_lines_wrapper($text, $lineItemsGroups, $lineItems);
    ...
    if ($task == 'pdf') {
        ...
        try {
            ...
            if ($task == 'pdf') {
                ...
                fclose($fp);
                $pdf->outputPDF($directoryName."/".$file_name, 'F');
            }
...
function populate_group_lines_wrapper($text, $lineItemsGroups, $lineItems, $element = 'table')
{
    $firstValue = '';
    $firstNum = 0;
    ...
    if ($firstValue !== '' && $lastValue !== '') {
        ...
        if (count($lineItemsGroups) != 0) {
            ...
            foreach ($lineItemsGroups as $group_id => $lineItemsArray) {
                $groupPartTemp = populate_product_lines_wrapper($groupPart, $lineItemsArray);
                $groupPartTemp = populate_service_lines_wrapper($groupPartTemp, $lineItemsArray);
        ...
        $text = populate_product_lines_wrapper($text, $lineItems);
        $text = populate_service_lines_wrapper($text, $lineItems);
    }
...
function populate_product_lines_wrapper($text, $lineItems, $element = 'tr')
{
    $firstValue = '';
    $firstNum = 0;
...
function populate_service_lines_wrapper($text, $lineItems, $element = 'tr')
{
    $firstValue = '';
    $firstNum = 0;
